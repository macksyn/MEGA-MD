...
if (MONGO_URL) {
  try {
    ...
    activityStatsSchema.index({ groupId: 1, month: 1 })
    activityStatsSchema.index({ userId: 1, groupId: 1, month: 1 })

    mongoose.connect(MONGO_URL).catch(err => console.error('[MONGO] Connection error:', err))

    // Attendance User Stats Schema
    const attendanceUserStatsSchema = new mongoose.Schema({
      userId: { type: String, required: true, index: true, unique: true },
      lastAttendance: { type: String }, // Format: DD-MM-YYYY
    ...
    const AttendanceUserStats = mongoose.model('AttendanceUserStats', attendanceUserStatsSchema)

    // Attendance Records Schema
    const attendanceRecordSchema = new mongoose.Schema({
      userId: { type: String, required: true, index: true },
      date: { type: String, required: true, index: true }, // Format: DD-MM-YYYY
      extractedData: {
        name: String,
        location: String,
        time: String,
        ...
        dob: String,
        relationship: String,
        wakeUpMembers: [String],
        parsedBirthday: mongoose.Schema.Types.Mixed
      },
    ...
    const birthdaySchema = new mongoose.Schema({
      ...
      updateHistory: [{
        type: String,
        previousName: String,
        previousBirthday: mongoose.Schema.Types.Mixed,
        newName: String,
        newBirthday: mongoose.Schema.Types.Mixed,
        timestamp: Date
      }]
    })

    birthdaySchema.index({ 'birthday.searchKey': 1 })
    ...
    adapters.mongo = {
      ...
      getModel(name) {
        return dynamicModels[name] || mongoose.models[name]
      },
      ...
      // Attendance User Stats Methods
      async getAttendanceUserStats(userId) {
        try {
          return await AttendanceUserStats.findOne({ userId }).lean()
        } catch (e) {
          console.error('[MONGO] Get attendance user stats error:', e.message)
          return null
        }
      },

      async updateAttendanceUserStats(userId, updates) {
        try {
          await AttendanceUserStats.updateOne(
            { userId },
          ...
          console.error('[MONGO] Update attendance user stats error:', e.message)
          return false
        }
      },
      ...
      async saveAttendanceRecord(record) {
        try {
          ...
          console.error('[MONGO] Save attendance record error:', e.message)
          return false
        }
      },

      async getAttendanceRecords(userId, limit = 10) {
        try {
          ...
          console.error('[MONGO] Get attendance records error:', e.message)
          return []
        }
      },

      async getAttendanceRecordsByDate(date) {
        try {
          return await AttendanceRecord.find({ date }).lean()
        } catch (e) {
          console.error('[MONGO] Get attendance records by date error:', e.message)
          return []
        }
      },

      async deleteOldAttendanceRecords(cutoffDate) {
        try {
          ...
          console.error('[MONGO] Delete old attendance records error:', e.message)
          return 0
        }
      },

      // Birthday Methods
      async saveBirthday(birthdayData) {
        try {
          await Birthday.replaceOne(
          ...
          console.error('[MONGO] Save birthday error:', e.message)
          return false
        }
      },
      ...
      async getBirthday(userId) {
        try {
          ...
          console.error('[MONGO] Get birthday error:', e.message)
          return null
        }
      },

      async getBirthdaysBySearchKey(searchKey) {
        try {
          return await Birthday.find({ 'birthday.searchKey': searchKey }).lean()
        } catch (e) {
          console.error('[MONGO] Get birthdays by search key error:', e.message)
          return []
        }
      },

      // ========== ACTIVITY STATS METHODS (NEW) ==========
      async saveActivityStats(activityId, userId, groupId, month, updates) {
        try {
          ...
          console.error('[MONGO] Save activity stats error:', e.message)
        }
      },

      async getActivityStats(activityId) {
        try {
          return await ActivityStats.findOne({ activityId }).lean()
        } catch (e) {
          console.error('[MONGO] Get activity stats error:', e.message)
          return null
        }
      },

      async getGroupActivityStats(groupId, month, limit = 1000) {
        try {
          return await ActivityStats.find({ groupId, month })
            .sort({ points: -1 })
            .limit(limit)
            .lean()
        } catch (e) {
          console.error('[MONGO] Get group activity stats error:', e.message)
          return []
        }
      },
    ...
    printLog('store', 'MongoDB enabled - Unlimited message storage with full data persistence + Activity tracking')
  } catch (e) {
...
/**
* ----------------- PostgreSQL (Activity support to be added) -----------------
*/
...
if (backend === 'memory' && POSTGRES_URL) {
  try {
    ...
    adapters.postgres = {
      ...
      // Activity stats fallback to settings for PostgreSQL
      async saveActivityStats(activityId, userId, groupId, month, updates) {
        try {
          const statsKey = `activity_stats_${month}`
          const groupStats = await this.getSetting(groupId, statsKey) || {}
          groupStats[userId] = { ...groupStats[userId], ...updates, updatedAt: new Date() }
          await this.saveSetting(groupId, statsKey, groupStats)
        } catch (e) {
          console.error('[POSTGRES] Save activity stats error:', e.message)
        }
      },

      async getActivityStats(activityId) {
        try {
          const [userId, groupId, month] = activityId.split('_')
          const statsKey = `activity_stats_${month}`
          const groupStats = await this.getSetting(groupId, statsKey) || {}
          return groupStats[userId] || null
        } catch (e) {
          console.error('[POSTGRES] Get activity stats error:', e.message)
          return null
        }
      },
      ...
      async getGroupActivityStats(groupId, month, limit = 1000) {
        try {
          const statsKey = `activity_stats_${month}`
          const groupStats = await this.getSetting(groupId, statsKey) || {}
          return Object.entries(groupStats).map(([userId, data]) => ({
            activityId: `${userId}_${groupId}_${month}`,
            userId,
            groupId,
            month,
            ...data
          ...
          console.error('[POSTGRES] Get group activity stats error:', e.message)
          return []
        }
      },
    ...
    adapters.mysql = {
      ...
      async saveActivityStats(activityId, userId, groupId, month, updates) {
        try {
          ...
          console.error('[MySQL] Save activity stats error:', e.message)
        }
      },
      ...
      async getActivityStats(activityId) {
        try {
          ...
          console.error('[MySQL] Get activity stats error:', e.message)
          return null
        }
      },
      ...
      async getGroupActivityStats(groupId, month, limit = 1000) {
        try {
          ...
          console.error('[MySQL] Get group activity stats error:', e.message)
          return []
        }
      },
    ...
    adapters.sqlite = {
      ...
      async saveActivityStats(activityId, userId, groupId, month, updates) {
        try {
          ...
          console.error('[SQLite] Save activity stats error:', e.message)
        }
      },
      ...
      async getActivityStats(activityId) {
        try {
          ...
          console.error('[SQLite] Get activity stats error:', e.message)
          return null
        }
      },
      ...
      async getGroupActivityStats(groupId, month, limit = 1000) {
        try {
          ...
          console.error('[SQLite] Get group activity stats error:', e.message)
          return []
        }
      },
...
const store = {
  ...
  async saveActivityStats(activityId, userId, groupId, month, updates) {
    if (backend === 'memory') {
      ...
      try {
        await adapters[backend].saveActivityStats(activityId, userId, groupId, month, updates)
      } catch (e) {
        console.error(`[STORE] Failed to save activity stats:`, e.message)
      }
    }
  },
  ...
  async getActivityStats(activityId) {
    if (backend === 'memory') {
      ...
      try {
        return await adapters[backend].getActivityStats(activityId)
      } catch (e) {
        console.error(`[STORE] Failed to get activity stats:`, e.message)
        return null
      }
    }
  },
  ...
  async getGroupActivityStats(groupId, month, limit = 1000) {
    if (backend === 'memory') {
      ...
      try {
        return await adapters[backend].getGroupActivityStats(groupId, month, limit)
      } catch (e) {
        console.error(`[STORE] Failed to get group activity stats:`, e.message)
        return []
      }
    }
  },
  ...
  async getAttendanceUserStats(userId) {
    if (backend === 'memory') {
      return await this.getSetting('global', `attendance_user_${userId}`)
    } else {
      try {
        return await adapters[backend].getAttendanceUserStats(userId)
      } catch (e) {
        console.error('[STORE] Failed to get attendance user stats:', e.message)
        return null
      }
    }
  },
  ...
  async updateAttendanceUserStats(userId, updates) {
    if (backend === 'memory') {
      const current = await this.getAttendanceUserStats(userId) || {}
      await this.saveSetting('global', `attendance_user_${userId}`, { 
        ...current, 
        ...updates,
        updatedAt: new Date()
      })
      ...
      try {
        return await adapters[backend].updateAttendanceUserStats(userId, updates)
      } catch (e) {
        console.error('[STORE] Failed to update attendance user stats:', e.message)
        return false
      }
    }
  },
  ...
  async saveAttendanceRecord(record) {
    if (backend === 'memory') {
      const records = await this.getSetting('global', 'attendance_records') || []
      records.push({ ...record, timestamp: new Date() })
      await this.saveSetting('global', 'attendance_records', records)
      return true
    } else {
      try {
        return await adapters[backend].saveAttendanceRecord(record)
      } catch (e) {
        console.error('[STORE] Failed to save attendance record:', e.message)
        return false
      }
    }
  },
  ...
  async getAttendanceRecords(userId, limit = 10) {
    if (backend === 'memory') {
      const allRecords = await this.getSetting('global', 'attendance_records') || []
      return allRecords
      ...
      try {
        return await adapters[backend].getAttendanceRecords(userId, limit)
      } catch (e) {
        console.error('[STORE] Failed to get attendance records:', e.message)
        return []
      }
    }
  },
  ...
  async getAttendanceRecordsByDate(date) {
    if (backend === 'memory') {
      ...
      try {
        return await adapters[backend].getAttendanceRecordsByDate(date)
      } catch (e) {
        console.error('[STORE] Failed to get attendance records by date:', e.message)
        return []
      }
    }
  },
  ...
  async deleteOldAttendanceRecords(cutoffDate) {
    if (backend === 'memory') {
      ...
      await this.saveSetting('global', 'attendance_records', filtered)
      return allRecords.length - filtered.length
    } else {
      try {
        return await adapters[backend].deleteOldAttendanceRecords(cutoffDate)
      } catch (e) {
        console.error('[STORE] Failed to delete old attendance records:', e.message)
        return 0
      }
    }
  },
  ...
  async saveBirthday(birthdayData) {
    if (backend === 'memory') {
      const birthdays = await this.getSetting('global', 'birthdays') || {}
      birthdays[birthdayData.userId] = birthdayData
      await this.saveSetting('global', 'birthdays', birthdays)
      return true
    } else {
      try {
        return await adapters[backend].saveBirthday(birthdayData)
      } catch (e) {
        console.error('[STORE] Failed to save birthday:', e.message)
        return false
      }
    }
  },
  ...
  async getBirthday(userId) {
    if (backend === 'memory') {
      ...
      try {
        return await adapters[backend].getBirthday(userId)
      } catch (e) {
        console.error('[STORE] Failed to get birthday:', e.message)
        return null
      }
    }
  },
  ...
  async getBirthdaysBySearchKey(searchKey) {
    if (backend === 'memory') {
      ...
      try {
        return await adapters[backend].getBirthdaysBySearchKey(searchKey)
      } catch (e) {
        console.error('[STORE] Failed to get birthdays by search key:', e.message)
        return []
      }
    }
  },
