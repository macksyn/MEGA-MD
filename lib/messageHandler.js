...
const activityTracker = require('../plugins/activitytracker');
const pluginLoader = require('./pluginLoader');

// Dynamic plugin handlers (loaded in index.js)
global.messagePlugins = global.messagePlugins || [];
...
const channelInfo = {
    contextInfo: {
        ...
        forwardedNewsletterMessageInfo: {
            newsletterJid: '120363404921992902@newsletter',
            newsletterName: 'Groq Test',
            serverMessageId: -1
        }
    }
};
...
async function handleMessages(sock, messageUpdate) {
    try {
        ...
        // Auto-detect attendance forms in groups
        if (isGroup && rawText && /GIST\s+HQ/i.test(rawText)) {
            try {
                const { attendance } = require('../plugins/attendance');
                const handleAutoAttendance = attendance.handleAutoAttendance || attendance;
                const handled = await handleAutoAttendance(message, sock);
                if (handled) return;
            } catch (e) {
                console.error('[ATTENDANCE] Auto-detection failed:', e);
            }
        }
        ...
        if (message.message?.stickerMessage) {
            ...
            if (fileSha256) {
                ...
                if (stickers[hash]) {
                    ...
                    if (foundCommand) {
                        ...
                        try {
                            await foundCommand.handler(sock, syntheticMessage, cmdArgs, stickerContext);
            ...
            // Sticker with no mapped command — still dispatch to plugins
            await pluginLoader.dispatchMessage(sock, message, { chatId, senderId, isGroup });
            return;
        }
        ...
        if (message.message?.buttonsResponseMessage) {
            ...
            if (buttonId === 'channel') {
                await sock.sendMessage(chatId, {
                    text: '*Join our Channel:*\n[https://whatsapp.com/channel/0029Vad8fY6HwXbB83yLIx2n](https://whatsapp.com/channel/0029Vad8fY6HwXbB83yLIx2n)'
                }, { quoted: message });
                return;
                ...
                await sock.sendMessage(chatId, {
                    text: `*Support*\n\nhttps://whatsapp.com/channel/0029Vad8fY6HwXbB83yLIx2n`
                }, { quoted: message });
                return;
            }
        }
        ...
        if (!message.key.fromMe) {
            ...
            try {
                await activityTracker.trackActivity(message);
            } catch (error) {
        ...
        // ── No prefix, no command → passive message handling + chatbot ──────────
        ...
        // ── Has prefix but no matching command → same passive handling ───────────
        if (!command) {
            if (isGroup) {
                await handleTagDetection(sock, chatId, message, senderId);
                await handleMentionDetection(sock, chatId, message);
        ...
        // ── Matched command — check permissions then execute ──────────────────────
        const botMode = await store.getBotMode();
        const isAllowed = (() => {
            if (isOwnerOrSudoCheck) return true;
            switch (botMode) {
